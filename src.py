...
train_df = train_df.sample(min(20000, len(train_df)), random_state=42)
test_df = test_df.sample(min(5000, len(test_df)), random_state=42)
...
plt.colorbar()
plt.xticks(range(len(labels_text)), labels_text, rotation=45)
plt.yticks(range(len(labels_text)), labels_text)
...
plt.title("Model Performance Metrics (Improved Visualization)", fontsize=14)
