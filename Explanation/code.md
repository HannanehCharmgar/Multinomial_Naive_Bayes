...
## بردارسازی متن (Text Vectorization)

بردارسازی متن یعنی تبدیل متن‌ها به یک **نمایش عددی** که مدل بتواند روی آن‌ها کار کند، چون الگوریتم‌ها نمی‌توانند مستقیم روی متن‌ها عملیات ریاضی انجام دهند.

### مثال ساده با CountVectorizer

فرض کنید دو متن داریم:

1. `"stock market up up"`  
2. `"market down trend"`  

CountVectorizer تعداد هر کلمه را می‌شمارد و بردار می‌سازد:
...
- هر متن به یک **بردار عددی** تبدیل شده که نشان‌دهنده تعداد تکرار هر کلمه در متن است.

> نکته: اگر بخواهیم وزن کلمات مهم‌تر نسبت به کلمات عمومی بیشتر شود، می‌توانیم از **TF-IDF** استفاده کنیم. اما در پروژه‌ی ما همان CountVectorizer کافی است.
