...
Score_Normal ∝ Prior_Normal × P(dear | Normal) × P(friend | Normal)
...
Score_Spam ∝ Prior_Spam × P(dear | Spam) × P(friend | Spam)


   با جایگذاری مقادیر (فرضاً): $0.33 \times (\text{مقادیر Likelihood اسپم}) \approx 0.01$.

## توضیح مرحله‌ای Multinomial Naive Bayes با مثال پیام "dear friend"

این مثال نشان می‌دهد چگونه **Multinomial Naive Bayes** با استفاده از **قانون بیز** تصمیم می‌گیرد که یک پیام به کدام کلاس تعلق دارد.

---

### ۱️⃣ قانون بیز (Bayes’ Theorem)
...
### ۳️⃣ محاسبه امتیاز (Score) برای هر کلاس
